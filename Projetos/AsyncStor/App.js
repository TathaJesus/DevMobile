...
import { View, Text, TextInput, Button, StyleSheet } from 'react-native'; // Importa componentes essenciais do React Native para construir a interface do usuário.
import AsyncStorage from '@react-native-async-storage/async-storage'; // Importa AsyncStorage, uma API para armazenamento persistente de dados simples no dispositivo.

export default function App() { // Define o componente principal da aplicação, chamado App.
  const [nome, setNome] = useState(''); // Declara uma variável de estado 'nome' e sua função 'setNome' para atualizá-la. O valor inicial é uma string vazia.
  const [mensagem, setMensagem] = useState(''); // Declara uma variável de estado 'mensagem' e sua função 'setMensagem' para exibir mensagens ao usuário. O valor inicial é uma string vazia.
  ...
  const salvarNome = async () => { // Define uma função assíncrona chamada 'salvarNome' para salvar o nome no AsyncStorage.
    try { // Inicia um bloco try-catch para lidar com possíveis erros durante a operação de salvamento.
      ...
      setMensagem('Erro ao salvar o nome: ' + erro); // Atualiza a mensagem com a descrição do erro, se houver.
    }
  };
  ...
  return ( // Inicia a seção de renderização do componente, que define a estrutura da interface do usuário.
    <View style={styles.container}>
      ...
      <TextInput
        ...
        value={nome} // Define o valor atual do campo como o valor da variável de estado 'nome'.
        onChangeText={setNome} // Define uma função que é chamada toda vez que o texto no campo de entrada muda, atualizando a variável de estado.
      />
...
const styles = StyleSheet.create({ // Cria um objeto de estilos usando StyleSheet.create, que otimiza os estilos para o React Native.
  container: { // Define estilos para o contêiner principal da aplicação.
    ...
    padding: 20, // Adiciona um preenchimento de 20 unidades dos lados.
  },
